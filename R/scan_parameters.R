...
scan_parameters <- function(
  ...
  if (!is.null(progress_file)) {
    if (!dir.exists(dirname(progress_file))) {
        cat(
            "Directory for progress file ", dirname(progress_file),
            " doesn't exist, creating...\n", sep = "")
        dir.create(dirname(progress_file))
    }
    ...
    dplyr::do({
      ...
      if (verbose) {
        cat(
          ...
          "  imaging_n_cells_per_well_mu = ", params$imaging_n_cells_per_well_mu[1], "\n",
          "  imaging_n_cells_per_well_var = ", params$imaging_n_cells_per_well_var[1], "\n",
...
#' Scan and evaluate simulated screens using future package
#'
#' For each combination of given parameters, generate a simulated dataset
#' fit a model to it and estimate the predictive accuracy.
#'
...
#'
#' @export
scan_parameters_future <- function(
    n_genes_in_library,
    gene_abundance_alpha,
    gene_hit_rate,
    n_wells_per_screen,
  ...
  parameter_sets |>
    ...
    dplyr::do({
      ...
      job <- future::future({
        ...
        batch_progress_file <- paste0(progress_file, "_", params$param_index[1])
        if (!is.null(progress_file)) {
          if (!file.exists(batch_progress_file)) {
            model_evaluation |>
              readr::write_tsv(file = batch_progress_file)
          } else {
            model_evaluation |>
              readr::write_tsv(
                file = batch_progress_file,
                append = TRUE)
          }
        }
